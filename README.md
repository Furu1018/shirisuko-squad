...
**開発を引き継ぐときは [ROADMAP.md](ROADMAP.md) を読むこと** — いまの状態・次にやること・触るときの注意がまとまっている。
...
- **3凸ボード** (既定ビュー): 1日 3 凸の枠にボスを置くと、有利コードの編成 (属性別編成タブの案) が入る。
  **同じニケは 3 凸のうち 1 度だけ**なので、枠をまたいだ被りをその場で出し、「外す / 譲る」どちらが得かを計算して解く。
  「被りなしで最大の 3 凸を探す」で全候補から合計最大の組み合わせを入れる
- **属性別編成タブ**: コードごとに本命の編成を 3 案まで保存し、同じ土俵 (癖なし) やボス条件で比べる
- **ユニオンレイドタブ**: ボス 5 枠 × デッキ 3 枠。ユニオンメンバー全員ぶんを一括計算し、盤面をコードで配る (運営者向け)
