...
describe('정밀 수치 내보내기', () => {
  ...
  it('구간마다 합계와 누적을 함께 적는다', () => {
    ...
    expect(rows[0]).toEqual(['開始(秒)', '終了(秒)', '리타', '크라운', '合計', '累積']);
  ...
  it('최종 표에 지분과 스쿼드 합계를 담는다', () => {
    ...
    expect(rows[3]).toEqual(['スカッド合計', 66, '', '', '', '', '100.00']);
  });
  ...
  it('한 장에 최종과 구간을 잇는다', () => {
    ...
    expect(text).toContain('条件,3돌 · 싱크로 400');
    expect(text).toContain('キャラクター,総ダメージ');
    expect(text).toContain('区間別ダメージ (1秒単位)');
    // 숫자는 줄이지 않고 1의 자리까지 그대로 적는다.
    expect(text).toContain('スカッド合計,66');
  });

  it('타임라인이 없는 결과도 최종만으로 내보낸다', () => {
    const text = damageCsv(result({ timeline: undefined }), ['리타']);
    expect(text).toContain('キャラクター,総ダメージ');
    expect(text).not.toContain('区間別ダメージ');
  });

  it('파일 이름에서 못 쓰는 글자를 턴다', () => {
    expect(csvFileName('덱 1', new Date(2026, 7, 31))).toBe('ニケ計算機_덱 1_20260831.csv');
    expect(csvFileName('a/b:c*', new Date(2026, 11, 5))).toBe('ニケ計算機_abc_20261205.csv');
    expect(csvFileName('   ', new Date(2026, 0, 9))).toBe('ニケ計算機_計算_20260109.csv');
  });
});
