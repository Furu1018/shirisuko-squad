...
export function squadPreview(
  ...
  decks.forEach((squad, index) => {
    ...
    if (many) row.append(el('span', 'share-deck-label', `デッキ ${index + 1}`));
...
export function mountSharePanel(hosts: SharePanelHosts, deps: SharePanelDeps): SharePanel {
  ...
  search.type = 'search';
  search.placeholder = '名前 · 投稿者 · 説明で検索';
  search.autocomplete = 'off';
  search.setAttribute('aria-label', '共有された設定を検索');
  ...
  function voteButton(item: ShareItem, want: 1 | -1): HTMLButtonElement {
    ...
    button.title = want === 1
      ? 'いいね — もう一度押すと取り消し' : 'いまいち — もう一度押すと取り消し';
    button.append(
  ...
  function renderList(): void {
    ...
    if (loading) {
      hosts.list.append(el('p', 'share-empty', '一覧を取得しています…'));
      return;
    }
    if (listError) {
      hosts.list.append(el('p', 'share-empty', `一覧を取得できませんでした — ${listError}`));
      const retry = el('button', 'share-upload-btn', '再試行');
      retry.type = 'button';
    ...
    if (items.length === 0) {
      hosts.list.append(el('p', 'share-empty', 'まだ投稿された設定がありません。最初の1件を投稿してみてください。'));
      return;
    }
  ...
  function renderRows(): void {
    ...
    if (shown.length === 0) {
      rowsHost.append(el('p', 'share-empty', '検索に一致する設定がありません。'));
      return;
    }
    ...
    for (const item of rankItems(shown)) {
      ...
      if (item.uses > 0) by.append(el('span', 'share-uses', ` · 適用 ${item.uses}`));
      body.append(by);

      const votes = el('div', 'vote-pill');
      votes.append(voteButton(item, 1), voteButton(item, -1));

      const apply = el('button', 'share-apply-btn', '適用');
      apply.type = 'button';
    ...
    rowsHost.append(el(
      'p', 'share-foot',
      `${shown.length}件 · 評価は IP ごとに1票です — もう一度押すと取り消し、反対側を押すと乗り換えです。`,
    ));
  }
  ...
  function renderUpload(): void {
    ...
    nameLabel.append(el('span', undefined, '名前 '), el('span', 'req', '*'));
    const name = el('input', 'share-input');
    name.type = 'text';
    name.maxLength = 40;
    name.placeholder = deps.kind === 'boss' ? '例: ユニレ ボス3 3日目' : '例: 水冷ボス デッキ1';
    ...
    autoLabel.append(
      el('span', undefined, '説明 '),
      el('span', 'opt', '· 今の設定から自動で作ります'),
    );
    const autoBox = el('div', 'share-auto-box', auto || '(設定が空です)');
    autoBox.dataset.shareAuto = '';
    autoField.append(autoLabel, autoBox);

    const byField = el('div', 'share-field');
    const byLabel = el('label');
    byLabel.append(el('span', undefined, '投稿者 '), el('span', 'opt', '· 空欄なら「匿名」'));
    ...
    const warn = el(
      'p', 'share-warn',
      '投稿後は自分で削除できません。名前と投稿者欄に個人情報を入れないでください。',
    );

    const submit = el('button', 'share-upload-btn', 'サーバーに投稿');
    submit.type = 'button';
    ...
    form.append(
      ...
      el('p', 'share-hint', '投稿を押したときだけサーバーに送信されます。それまでは何も送られません。'),
    );
  ...
  async function send(name: string, by: string, submit: HTMLButtonElement): Promise<void> {
    ...
    try {
      ...
      deps.notify(result.existed
        ? `同じ設定がすでに「${result.item.name}」として投稿されているため、その項目を使います。`
        : `「${result.item.name}」を投稿しました。`, true);
    } catch (error) {
      deps.notify(error instanceof Error ? error.message : String(error));
    } finally {
      submit.disabled = false;
      submit.textContent = 'サーバーに投稿';
    }
  }
